...
Snowflake-native Streamlit | v4.0
Deploy: Snowflake UI → Projects → Streamlit → + Streamlit App → paste → Run
"""
...
def get_session_token():
    """Extract the active session bearer token."""
    session = snowpark_context.get_active_session()
    return session.connection.rest.token


def call_cortex_analyst(question):
    """Call Cortex Analyst REST API using the session bearer token."""
    try:
        token = get_session_token()
    except Exception as e:
        return {"sql": None, "results": None, "error": f"Could not get session token: {e}", "answer_text": None}
    ...
    try:
        ...
        return {"sql": None, "results": None, "error": "Request timed out — try again.", "answer_text": None}
    except requests.exceptions.HTTPError as e:
        return {"sql": None, "results": None, "error": f"HTTP {resp.status_code}: {resp.text[:300]}", "answer_text": None}
    except Exception as e:
        return {"sql": None, "results": None, "error": str(e), "answer_text": None}

    blocks = data.get("message", {}).get("content", [])
    sql, answer_text, results, error = None, None, None, None
    ...
    for block in blocks:
        ...
        if t == "sql":
            ...
            error = block.get("message", "Unknown error from Cortex Analyst.")
    ...
    return {"sql": sql, "results": results, "error": error, "answer_text": answer_text}
...
def render_results_table(results):
    ...
    note = (
        f"<div style='font-size:0.72rem;color:#4A6278;margin-top:6px'>"
        f"Showing 50 of {len(results)} rows</div>"
...
# ── Suggested questions ────────────────────────────────────────────────────────
SUGGESTED = [
    ("Which site has the most critical alarms today?",  "أي موقع لديه أكثر الأعطال الحرجة اليوم؟"),
...
with st.expander("📊 Network Status Summary", expanded=True):
    if st.session_state.noc_data:
        ...
        st.markdown('<div class="err-box">Could not load V_NOC_DAILY_SUMMARY.</div>', unsafe_allow_html=True)
...
with col_input:
    user_input = st.text_input(
        ...
        placeholder="e.g. Show worst performing cell… أو اكتب سؤالك بالعربية",
        key="chat_input",
    )
with col_send:
